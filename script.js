...
let colorLibraryToggle = null; // Color library collapse toggle
...
function initEventListeners() {
    ...
    // Color library toggle event
    if (colorLibraryToggle) {
        colorLibraryToggle.addEventListener('click', function() {
...
function highlightColorAreas(color) {
    ...
    for (let y = 0; y < blocksY; y++) {
        for (let x = 0; x < blocksX; x++) {
            ...
            // Dynamically adjust font size based on pixel block size and zoom level
            const scaledSize = Math.min(sizeX, sizeY) * zoomLevel;
            const fontSize = Math.max(8, Math.min(scaledSize * 0.6, scaledSize - 2));
            // Ensure font size doesn't exceed pixel block size
            ...
            if (scaledSize > 12 && showColorLabels && showColorLabels.checked) {
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    colorLibraryToggle = document.querySelector('.color-library-toggle');
